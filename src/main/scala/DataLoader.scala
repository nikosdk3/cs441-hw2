...
class DataLoader(val vec: Word2Vec, val embeddings: INDArray, val batchSize: Int, val seqLen: Int, val stride: Int, spark: SparkSession) extends Serializable {
  ...
  def slidingWindowsWithEmbeddings(data: Array[Int]): RDD[DataSet] = {
    ...
    val windows = spark.sparkContext.parallelize(0 until data.length - (seqLen + stride) by stride).map { i =>
      ...
      val targetArray = Nd4j.zeros(1, vec.vocabSize(), seqLen)  // Set sequence length to match input
